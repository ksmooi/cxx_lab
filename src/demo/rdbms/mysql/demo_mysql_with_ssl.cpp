...
// Database class encapsulating CRUD operations on a MySQL database using Boost Asio for asynchronous operations with SSL.
...
public:
     ...
     * @param ssl_ctx Boost.Asio SSL context for handling encrypted connections.
    ...
    Database(asio::io_context& ctx,
        ...
        : logger_(spdlog::get("main")),
          ...
          db_(db),
          port_(port),
          connection_(std::make_optional<mysql::tcp_ssl_connection>(ctx, ssl_ctx))  // Initialize SSL connection
    {}

    /**
     * Asynchronously connect to the MySQL database with SSL.
     * It resolves the host, prepares the connection parameters, and performs the connection handshake with SSL.
     */
    boost::asio::awaitable<void> connect() {
        try {
            ...
            params.set_ssl(mysql::ssl_mode::require);  // Enforce SSL for the connection

            // Connect and perform SSL handshake
            co_await connection_->async_connect(*endpoints.begin(), params, asio::use_awaitable);

            SPDLOG_LOGGER_INFO(logger_, "Connected to the database successfully with SSL.");
...
private:
    ...
    asio::ssl::context& ssl_ctx_;  // SSL context for encrypted connections
    std::optional<mysql::tcp_ssl_connection> connection_;  // MySQL SSL connection object
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        asio::io_context ctx;
        asio::ssl::context ssl_ctx(asio::ssl::context::tls_client);  // Use TLS for SSL connections

        // Load SSL certificates (optional, depending on your setup)
        // ssl_ctx.load_verify_file("path_to_ca_certificate.pem");
        ...
        // Create a Database object with SSL support
        Database db(ctx, ssl_ctx, host, user, password, database, port);
...
// src/demo/rdbms/demo_mysql_with_ssl -o create --name user1 --email user1@mail.com
// src/demo/rdbms/demo_mysql_with_ssl -o read
// src/demo/rdbms/demo_mysql_with_ssl -o update --id 1 --email user1_updated@mail.com
// src/demo/rdbms/demo_mysql_with_ssl -o delete --id 1
