...
public:
     ...
     */
    AsyncDBHandler(asio::io_context& io_context) : io_ctx_(io_context) {}
    ...
    awaitable<void> disconnect() {
        return asio::async_compose<decltype(asio::use_awaitable), void()>(
            [this](auto& self) {
                asio::post(io_ctx_, [this, self = std::move(self)]() mutable {
                    handler_.disconnect();
                    self.complete();
                });
            },
    ...
    awaitable<bool> insert(const std::string& key, const std::string& value) {
        return asio::async_compose<decltype(asio::use_awaitable), void(bool)>(
            [this, key, value](auto& self) {
                asio::post(io_ctx_, [this, key, value, self = std::move(self)]() mutable {
    ...
    awaitable<bool> remove(const std::string& key) {
        return asio::async_compose<decltype(asio::use_awaitable), void(bool)>(
            [this, key](auto& self) {
                asio::post(io_ctx_, [this, key, self = std::move(self)]() mutable {
    ...
    awaitable<std::size_t> query(const std::vector<std::string>& keys, std::vector<std::string>& values) {
        return asio::async_compose<decltype(asio::use_awaitable), void(std::size_t)>(
            [this, keys, values](auto& self) {
                asio::post(io_ctx_, [this, keys, values, self = std::move(self)]() mutable {
...
private:
    asio::io_context& io_ctx_; ///< Reference to the Boost.Asio I/O context
 ...
 * 
 * This function sets up the Boost.Asio I/O context, handles system signals
 * for graceful termination, initializes the asynchronous database handler,
 * spawns the sample_operations coroutine, and runs the I/O context on a separate thread.
 * 
